...
class TrackDAO(BaseDbDAO):
    ...
    def get_tracks_by_name_artist_name(name : str, artist_name : str) -> List[Track]:
        track_artist_rows = session.query(track_artist).join(Track).join(Artist).filter(Track.name == name).filter(Artist.name == artist_name).first()
    ...
    @staticmethod
    def get_tracks_by_name_artist_name_release_name(name : str, artist_name : str, release_name : str) -> Optional[List[Track]]:
        track_rows = session.query(track_artist).join(Track).join(Artist).join(SpotifyTrack).join(Release).filter(Track.name == name).filter(Artist.name == artist_name).filter(Release.name == release_name).first()
