...
class TrackMetricsSnapshotDAO(BaseDbDAO):
    @staticmethod
    def add_track_metrics(track_id: int, playcount: int) -> None:
        ...
        if track_metrics is None:
            track_metrics = TrackMetricsSnapshot(
                track_id=track_id, playcount=playcount, collected_at=datetime.now()
            )
            session.add(track_metrics)

    def update_track_metrics(id: int, playcount: int) -> None:
    ...
    def get_track_metrics_by_id(id: int) -> TrackMetricsSnapshot:
        return (
            session.query(TrackMetricsSnapshot)
            .fiter(TrackMetricsSnapshot.id == id)
            .first()
        )
    ...
    def get_track_metrics_by_track_id(track_id: int) -> TrackMetricsSnapshot:
        return (
            session.query(TrackMetricsSnapshot)
            .filter(TrackMetricsSnapshot.track_id == track_id)
            .first()
        )
...
class ArtistMetricsSnapshotDAO(BaseDbDAO):
    @staticmethod
    def add_artist_metrics(artist_id: int, monthly_listeners: int) -> None:
        artist_metrics = ArtistMetricsSnapshotDAO.get_artist_metrics_by_artist_id(
            artist_id
        )
    ...
    def update_artist_metrics(id: int, monthly_listeners: int) -> None:
    ...
    def get_artist_metrics_by_id(id: int) -> ArtistMetricsSnapshot:
        return (
            session.query(ArtistMetricsSnapshot)
            .filter(ArtistMetricsSnapshot.id == id)
            .first()
        )
    ...
    def get_artist_metrics_by_artist_id(artist_id: str) -> ArtistMetricsSnapshot:
        return (
            session.query(ArtistMetricsSnapshot)
            .filter(ArtistMetricsSnapshot.artist_id == artist_id)
            .first()
        )
