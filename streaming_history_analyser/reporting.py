...
def artists_data_to_csv(session, output_file="data/csv/artists_data.csv"):
    ...
    fieldnames = [
        ...
        'Minutes_Streamed', 'Click_Row_Count', 'Skipped_Count', 'Track_Done_Count',
        'Track_Count', 'Single_Number', 'Album_Number', 'EP_Number'
    ]
    ...
    for row in stream_q:
        stats[row.artist_id].update({
            'Name': row.name,
            'Genres': ",".join(row.genres) if isinstance(row.genres, (list, tuple)) else row.genres,
...
def tracks_data_to_csv(session, output_file="data/csv/tracks_data.csv"):
    ...
    # Define the output columns
    fieldnames = [
        'Name', 'Duration_Ms', "First_Played_At", "Last_Played_At", "Track_Done_Count", "Skipped_Count", "Click_Row_Count", "Minutes_Streamed"
    ]
...
def releases_data_to_csv(session, output_file="data/csv/releases_data.csv"):
    # Define the output columns
    fieldnames = [
        'Name', 'Release_Type', "Popularity", 'Total_Duration_m', "Release_Date", "Total_Tracks", "Track_Done_Count", "Skipped_Count", "Click_Row_Count", "Minutes_Streamed"
    ]
