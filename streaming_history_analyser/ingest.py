...
def load_streaming_history_folder():
    ...
    for filename in filenames:
        if not re.search(r'^Streaming_History_Audio_(?:\d{4}(?:-\d{4})*)_\d{1,2}\.json$', filename):
            raise ValueError(f"The file name {filename} is not correct, you should not rename the files sent by Spotify.") 
        
    pattern = re.compile(r'(\d{1,2})(?=\.json$)')
    filenames = sorted(
        filenames,
        key=lambda fn: int(pattern.search(fn).group(1))
    )
