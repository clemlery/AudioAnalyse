...
for uploaded_file in uploaded_files:
    if not re.search(r'^Streaming_History_Audio_(?:\d{4}(?:-\d{4})*)_\d{1,2}\.json$', uploaded_file.name):
        raise ValueError(f"The file name {uploaded_file.name} is not correct, you should not rename the files sent by Spotify.") 
