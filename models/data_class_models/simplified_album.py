...
class SimplifiedAlbum(BaseModel):
    ...
    def check_release_date_matches_precision(self):
        ...
        if precision == "year" and not re.fullmatch(r"\d{4}", date):
            raise ValueError(
                'Pour precision "year", release_date doit être au format YYYY'
            )
        if precision == "month" and not re.fullmatch(r"\d{4}-\d{2}", date):
            raise ValueError(
                'Pour precision "month", release_date doit être au format YYYY-MM'
            )
        if precision == "day" and not re.fullmatch(r"\d{4}-\d{2}-\d{2}", date):
            raise ValueError(
                'Pour precision "day", release_date doit être au format YYYY-MM-DD'
            )
        return self
